...
using SteamRelisesAnalytics.ParserSandBox;

var httpClient = new HttpClient();

List<AppReleaseDataToSave> appReleaseDatasToSave = new();

Task SaveReleaseItem(string appId, string appTitle, int year, int month, int? dayOfMonth)
{
    appReleaseDatasToSave.Add(new AppReleaseDataToSave(int.Parse(appId), appTitle, year, month, dayOfMonth));
...
async Task Parse()
{
    ...
    while(true)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            var checkReleaseDateResult = Tools.Check(releaseItem.ReleaseDate, correctYear, fromMonth, toMonth);
            if (checkReleaseDateResult.PositionCheckResult == Tools.PositionCheckResult.TooFuture)
            {
                isTooFuture = true;
                break;
            }

            if (checkReleaseDateResult.PositionCheckResult == Tools.PositionCheckResult.TooPast)
            {
                isTooPast = false;
                break;
            }

            if (checkReleaseDateResult.PositionCheckResult == Tools.PositionCheckResult.SkipThis)
            {
                continue;
            }

            if (checkReleaseDateResult.PositionCheckResult != Tools.PositionCheckResult.Correct ||
                checkReleaseDateResult.Month.HasValue == false)
            {
                throw new InvalidOperationException("такого быть не должно");
            }

            await SaveReleaseItem(releaseItem.Appid, releaseItem.Appid, correctYear, checkReleaseDateResult.Month.Value, checkReleaseDateResult.DayOfMonth);
        }
...
var appReleaseDatas = new List<AppReleaseData>();

HashSet<string> uniqueReleaseDates = new(appReleaseDatas.Select(ard=>ard.ReleaseDate));

for (int si = 0; si < 30000; si+=100)
{
    string url =
        $"https://store.steampowered.com/search/results/?query=" +
        $"&start={si}" +
    ...
    var appReleases = items.Select(item =>
    {
        var releaseData = item.SelectSingleNode(".//div[contains(@class, 'search_released')]").InnerHtml.Trim();
        var appid = item.Attributes.First(attr => attr.Name == "data-ds-appid").Value;
        var title = item.SelectSingleNode(".//span[contains(@class, 'title')]").InnerHtml.Trim();
    ...
    appReleaseDatas.AddRange(appReleases);
    
    await Task.Delay(500);
}

/*var unique_release_dates = appReleases.Select(ar => ar.releaseData).Distinct();

var nodes = hd.DocumentNode.SelectNodes("//div[contains(@class, 'search_released')]");
var normalizedResultsHtml = resultsHtml.Replace("\\n", "\n").Replace("\\r", "\r").Replace("\\t", "\t").Replace("\\\"", "\"");*/

int a = 1;

record AppReleaseData(string ReleaseDate, string Appid, string Title);

record AppReleaseDataToSave(int AppId, string AppTitle, int Year, int Month, int? DayOfMonth);